...
(async () => {
  ...
  await page.waitForTimeout(300);
  ok(await page.locator('#zoekVeld').count() === 1, 'de zoekbak opent met een invoerveld');
  ok(await page.locator('#zoekVeld').evaluate((e) => document.activeElement === e), 'het veld heeft meteen de cursor');
  ...
  // ---- 5. het scherm: groepen met kopjes, hoogstens vijf per groep ----
  await page.fill('#zoekVeld', 'casa');
  await page.waitForTimeout(300);
  ok(await page.locator('#zoekUit .zoekkop').count() > 0, 'de uitslag staat gegroepeerd met kopjes');
  const perGroep = await page.evaluate(() => {
    const uit = [];
    let n = 0;
    Array.from(document.querySelectorAll('#zoekUit > *')).forEach(function (el) {
      if (el.classList.contains('zoekkop')) { if (n) uit.push(n); n = 0; } else n++;
    });
    if (n) uit.push(n);
    return uit;
  });
  ok(perGroep.every(function (n) { return n <= 5; }), 'nooit meer dan vijf per groep (' + perGroep.join(',') + ')');
  ...
  await page.fill('#zoekVeld', 'estar');
  await page.waitForTimeout(400);
  await page.fill('#zoekVeld', 'winkel');
  await page.waitForTimeout(400);
  ...
  const naar = await page.evaluate(() => ({
    ...
    dicht: document.getElementById('zoekWrap').classList.contains('hidden')
  }));
  ok(naar.tab === true, 'een grammaticatreffer opent de grammaticapagina');
  ok(naar.dicht === true, 'en de zoekbak gaat dicht');
